//
//  KNNDTW.swift from Github
//

import Foundation

open class KNNDTW: NSObject {
    
    
    //meta parameters
    fileprivate var n_neighbors: Int = 0
    fileprivate var max_warping_window: Int = 1000 // this isn't implemented yet
    fileprivate var unique_labels: [String] = [String]()
    
    
    //internal data structures
    fileprivate var curves: [[Float]] = [[Float]]()
    fileprivate var labels: [String] = [String]()
    
    fileprivate var curve_label_pairs: [knn_curve_label_pair] = [knn_curve_label_pair]()
    
    open func configure(_ neighbors: Int, max_warp: Int) {
    ...
    open func train(_ data_sets: [knn_curve_label_pair]) {
    ...
    fileprivate func dtw_cost(_ y: [Float], x: [Float]) -> Float {
        ...
        while (i > 0 && j > 0) {
            if (i==0) {
                ...
                let tmp = min(acc_cost[i-1][j-1], acc_cost[i-1][j], acc_cost[i][j-1])
                if ( acc_cost[i-1][j] == tmp ) {
                    i = i - 1
                }
                else if (acc_cost[i][j-1] == tmp) {
                    j = j - 1
                }
                else {
                    i = i - 1
    ...
    open func predict(_ curve_to_test: [Float]) -> knn_certainty_label_pair {
    ...
    fileprivate struct knn_distance_label_pair {
        let distance: Float
        let label: String
    }
